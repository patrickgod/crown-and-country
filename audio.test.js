...
import {GameAudio,sanitizeSettings,AUDIO_FILES} from './audio.js';
...
class Context{
 ...
 createGain(){return {gain:new Parameter(),connect(){}};}
...
test('failed downloads can be retried and do not break gameplay calls',async()=>{let fail=true;const {audio}=setup({fetcher:async()=>({ok:!fail,arrayBuffer:async()=>new ArrayBuffer(1)})});await audio.play('place');assert.equal(audio.failures.length,6);assert.equal(audio.musicSource,undefined);fail=false;await audio.retry();assert.equal(audio.failures.length,0);assert.ok(audio.musicSource);});
